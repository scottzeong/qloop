...
## v4.28 진도율 수정 + 그룹 세부자료 UI + 질문 프롬프트 개선
...
## AI 분석 JSON 파싱 실패 근본 수정 (v4.44)
- [x] 파싱 실패 시 전체 응답 서버 로그에 출력 (원인 파악)
- [x] 잘린 JSON 자동 복구 로직 추가 (마지막 완전한 } 위치까지 파싱 재시도)
- [x] stripped 변수를 try 바깥으로 이동하여 catch 블록에서 접근 가능하도록 수정
